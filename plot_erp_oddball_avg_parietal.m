...
function h = plot_erp_oddball_avg_parietal(epochs, channel)
...
xmin = epochs{1}{1}.xmin;
xmax = epochs{1}{1}.xmax;
...
% drawing figure
newfig=1
if newfig, h = figure('units', 'pixels', 'Position', [300 400 600 425], 'Color', 'w');
else h = NaN; end
...
% setting plot limits
ylim([ymin-2, ymax+2])%ELENA: ylim([-max(abs([ymax ymin])), max(abs([ymax ymin]))])
xlim([xmin xmax]);
...
numnondiffcurves = size(erps, 1)
plotdiff=0
        legendtext = [];
        labels = {'standard', 'oddball'}
        for i = 1:2
            legendtext = [legendtext, '\color[rgb]{' num2str(colors(i,:)) '}' labels{i}];
